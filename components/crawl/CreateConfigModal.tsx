...
const configSchema = z.object({
  ...
  viewport_width: z.number().min(320).max(3840).default(1920),
  viewport_height: z.number().min(240).max(2160).default(1080),
  headless: z.boolean().default(true),
  timeout: z.number().min(1000).max(120000).default(30000),
  max_depth: z.number().min(1).max(10).default(1),
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Crawl Configuration</DialogTitle>
          <DialogDescription>
            Define a preset configuration for web crawling behavior
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                />
                {errors.viewport_width && (
                  <p className="text-sm text-destructive mt-1">
                    {errors.viewport_width.message}
                  </p>
                )}
              </div>
